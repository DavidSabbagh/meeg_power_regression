...
# save results
np.savetxt(op.join(cfg.path_outputs, 'simuls/synth_da/scores.csv'),
           results, delimiter=',')
np.savetxt(op.join(cfg.path_outputs, 'simuls/synth_da/names.csv'),
           np.array(list(pipelines)), fmt='%s')
np.savetxt(op.join(cfg.path_outputs, 'simuls/synth_da/distance_a.csv'),
           distances)
