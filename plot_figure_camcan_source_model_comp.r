...
data <- np$load(
  "./outputs/all_scores_models_camcan_source_mae_shuffle-split.npy",
...
data_comp_scores <- read.csv(
  "outputs/camcan_source_component_scores.csv",
  stringsAsFactor = T
)
...
fname <- "./outputs/fig_camcan_source_model_comp"
...
fname <- "./outputs/fig_camcan_source_component_selection"
ggsave(paste0(fname, ".png"),
       width = 8, height = 5, dpi = 300)
