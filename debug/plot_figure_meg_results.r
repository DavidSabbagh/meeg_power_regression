...
data <- np$load(
  "./outputs/all_scores_mag_models_mnecommonsubjects.npy",
  allow_pickle = T)[[1]] %>%
  as.data.frame()

data["mne"] <- np$load(
  "./outputs/scores_mag_models_mne_common.npy",
  allow_pickle = T)[[1]]["mne"] %>%
  as.data.frame()
...
ggsave("./figures/fig1_meg_data.png", width = 8, height = 6, dpi = 300)
ggsave("./figures/fig1_meg_data.pdf", width = 8, height = 6, dpi = 300)
