...
distance_A_id = 1  # Parameter 'mu': distance from A to Id
...
# save results
np.savetxt(op.join(cfg.path_outputs, 'simuls/synth_snr/scores.csv'),
           results, delimiter=',')
np.savetxt(op.join(cfg.path_outputs, 'simuls/synth_snr/names.csv'),
           np.array(list(pipelines)), fmt='%s')
np.savetxt(op.join(cfg.path_outputs,
           'simuls/synth_snr/sigmas.csv'),
           sigmas)
